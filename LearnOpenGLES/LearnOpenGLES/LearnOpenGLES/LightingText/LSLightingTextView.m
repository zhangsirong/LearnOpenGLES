...
@interface LSLightingTextView ()
{
    ...
    GLuint _emissionMap;//放射光贴图
}

@end

@implementation LSLightingTextView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if(self = [super initWithFrame:frame]){
        ...
        //着色器程序
        _shaderProgram = [LSShaderTool getShaderWithVertexShaderFile:@"LightingText" fragmentShaderFile:@"LightingText"];
        ...
        // 加载纹理第三个纹理
        spriteImage = [UIImage imageNamed:@"matrix.jpg"].CGImage;
        width = CGImageGetWidth(spriteImage);
        height = CGImageGetHeight(spriteImage);
        spriteContext = CGBitmapContextCreate(spriteData, width, height, 8, width*4, CGImageGetColorSpace(spriteImage), kCGImageAlphaPremultipliedLast);
        CGContextDrawImage(spriteContext, CGRectMake(0, 0, width, height), spriteImage);
        ...
        glActiveTexture(GL_TEXTURE2);
        glBindTexture(GL_TEXTURE_2D, _specularMap);
        glUniform1i(glGetUniformLocation(_shaderProgram, "material.emission"), 2);
...
- (void)render:(CADisplayLink *)displayLink
{
    ...
    // 设置材质
    glUniform1f(glGetUniformLocation(_shaderProgram, "material.shininess"), 8.0);
